...
- (void)show {
    ...
    [self.pickerView selectRow:[self.dayArr indexOfObject:@"23日"] inComponent:2 animated:YES];
}
...
- (void)saveBtnClick {
    ...
    self.selectStr = [NSString stringWithFormat:@"%ld年%@月%@日", [self.year integerValue], month, day];
    NSString *no = [NSString stringWithFormat:@"%ld%@%@", [self.year integerValue], month, day];
    if ([self.delegate respondsToSelector:@selector(datePickerViewSaveBtnClickDelegate:noTimer:)]) {
        [self.delegate datePickerViewSaveBtnClickDelegate:self.selectStr noTimer:no];
    }
}
/// 取消按钮点击方法
- (void)cancelBtnClick {
