...
@interface ShiPinListController ()<UITableViewDelegate,UITableViewDataSource,UIScrollViewDelegate,WMPlayerDelegate>

@property(nonatomic,strong)VideoCell *currentCell;
@property(nonatomic,strong)WMPlayer *wmPlayer;
@property(nonatomic,strong)NSMutableArray<NewVideoModel *> *dataSource;
@property (nonatomic, strong) UITableView *table;

@end

@implementation ShiPinListController
...
- (void)viewDidLoad {
    ...
    self.table.height = self.view.height - 64 - 49;
...
-(void)loadData{
    ...
    //拉取首页 猜你喜欢
    NSString *likeUrl = [NSString stringWithFormat:@"%@%d",ShipingList,self.type];
    
    [[AFNetWW sharedAFNetWorking] AFWithPostORGet:@"get" withURLStr:likeUrl WithParameters:nil success:^(id responseDic)
     {
         
         self.dataSource = (NSMutableArray *)[NewVideoModel objectArrayWithKeyValuesArray:responseDic];
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    cell.startPlayVideoBlcok = ^(UIImageView *backgroundIV, NewVideoModel *videoModel) {
        [weakSelf releaseWMPlayer];
        weakSelf.currentCell = (VideoCell *)backgroundIV.superview.superview;
        
        NewVideoModel *m = weakSelf.dataSource[indexPath.row];
        
        WMPlayerModel *playerModel = [WMPlayerModel new];
        playerModel.title = videoModel.title;
        playerModel.videoURL = [NSURL URLWithString:m.mp4_url];
