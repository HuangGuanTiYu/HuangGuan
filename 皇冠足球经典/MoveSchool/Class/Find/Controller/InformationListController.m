...
- (void) setUpData
{
    NSString *likeUrl = [NSString stringWithFormat:@"%@%@",BNewsList,self.ID];
    
    [[AFNetWW sharedAFNetWorking] AFWithPostORGet:@"get" withURLStr:likeUrl WithParameters:nil success:^(id responseDic)
     {
         NSArray *models = [InformationListModel objectArrayWithKeyValuesArray:responseDic];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    webVc.isZiXun = YES;
    HomePageModel *m = [[HomePageModel alloc] init];
    m.title = model.title;
    m.jumpUrl = model.jumpUrl;
    m.image = model.image;
    m.indate = model.publishdate;
    webVc.model = m;
